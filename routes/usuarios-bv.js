...
const { crearCuentaBV, 
        ...
        enviarCodigo,
        confirmarCorreo,
        codeConfirmEmail} = require('../controllers/usuario-controller');
...
usuarioRouter.get('/usuarios', [authVerifyToken, validarRoleBv], mostrarUsuarios)

usuarioRouter.get('/cuenta', authVerifyToken ,mostrarUser)

usuarioRouter.post('/crear-cuenta', [
     existeUser,
     existeEmailBv,
     validarLongitud,
     check('email', 'Ingresa un correo electrónico válido').isEmail()
,validateResult], crearCuentaBV)

usuarioRouter.post('/loginaccount', confirmarLogin)

usuarioRouter.post('/usuario-favoritos/:id', authVerifyToken, agregarUsuarioFavoritos)
